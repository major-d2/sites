function initSplash() {
    gameState = "splash", resizeCanvas(), splash = new Elements.Splash(assetLib.getData("splash"), canvas.width, canvas.height), userInput.addHitArea("moreGames", butEventHandler, null, "rect", {
...
function initStartScreen() {
    ...
    userInput.addHitArea("showTutorial", butEventHandler, null, "image", a), userInput.addHitArea("moreGames", butEventHandler, null, "image", b), userInput.addHitArea("credits", butEventHandler, null, "image", c);
    var d = new Array(a, b, c);
    panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("scoreNumbers"), gameState, d, canvas.width, canvas.height), panel.startTween1(), previousTime = (new Date).getTime(), updateStartScreenEvent()
}
function initCreditsScreen() {
    ...
    var b = new Array(a);
    panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("scoreNumbers"), gameState, b, canvas.width, canvas.height), panel.startTween2(), previousTime = (new Date).getTime(), updateCreditsScreenEvent()
}
function initTutorial() {
    ...
    var b = new Array(a);
    panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("scoreNumbers"), gameState, b, canvas.width, canvas.height), panel.startTween1(), previousTime = (new Date).getTime(), updateTutorialEvent()
}
function initGame() {
    gameState = "game", gameTouchState = 0, shotsSinceLastPot = 1, gameTimer = startTime, streak = 0, levelNum = 0, 1 == audioType && (musicTween.kill(), musicTween = TweenLite.to(music, .2, {
        volume: 0,
        ease: "Linear.easeNone"
        ...
        aRect: [0, 0, canvas.width, canvas.height]
    }, !0), hud = new Elements.Hud(assetLib.getData("hud"), assetLib.getData("timeNumbers"), assetLib.getData("tableNumbers"), {
        racks: 1,
        score: 0,
        multiplier: 10,
        balls: 0,
    ...
    var a = new Array;
    aBalls = new Array, aHoles = new Array;
    for (var b = 0; b < aLevelData[levelNum].aData.length; b++)
        if ("cueBall" == aLevelData[levelNum].aData[b].type)
            ...
            }, ballCallback, canvas.width, canvas.height), aBalls.push(cueBall);
        else if ("hole" == aLevelData[levelNum].aData[b].type)
            aHoles.push({
                x: aLevelData[levelNum].aData[b].p0.x,
                y: aLevelData[levelNum].aData[b].p0.y
            });
            ...
            "wall" == aLevelData[levelNum].aData[b].type && a.push({
    ...
    oPosData = {
        ...
        startStageX: 0,
        startStageY: 0
    }, linePredictor = new Utils.LinePredictor(a, aBalls, cueBall), table = new Elements.Table(assetLib.getData("table"), canvas.width, canvas.height), arrow = new Elements.Arrow(assetLib.getData("arrow"), assetLib.getData("cue"), canvas.width, canvas.height), aimX = targAimX = cueBall.startX, aimY = targAimY = cueBall.startY + oPosData.stageY, physics2D = new Utils.Physics2D(a, aBalls), hideNewRackIntro(this), playSound("rack"), previousTime = (new Date).getTime(), updateGameEvent()
}
function initNewRack() {
    levelNum = (levelNum + 1) % aLevelData.length, hud.oData.racks++, gameTimer += 15, aBalls = new Array;
    ...
    gameTouchState = 0, oPosData = {
        ...
        startStageX: 0,
        startStageY: 0
    }, aimX = targAimX = cueBall.startX, aimY = targAimY = cueBall.startY + oPosData.stageY, arrow.renderFunc = arrow.renderAim, linePredictor.aBalls = aBalls, linePredictor.cueBall = cueBall, physics2D.aBalls = aBalls, showNewRackIntro(), playSound("rack")
}
function showNewRackIntro() {
    newRackStart = !0, newRackY = -400, TweenLite.to(this, .5, {
        newRackY: 0,
        ease: "Back.easeOut",
...
function hideNewRackIntro(a) {
    newRackStart = !0, newRackY = 0, TweenLite.to(a, .5, {
        delay: .5,
        newRackY: 800,
        ease: "Back.easeIn",
        onComplete: function() {
...
function butEventHandler(a, b) {
    ...
    case "langSelect":
        ...
        break;
    case "showTutorial":
        playSound("hit1"), userInput.removeHitArea("showTutorial"), userInput.removeHitArea("moreGames"), userInput.removeHitArea("credits"), initTutorial();
        break;
    case "credits":
        playSound("hit1"), userInput.removeHitArea("showTutorial"), userInput.removeHitArea("moreGames"), userInput.removeHitArea("credits"), initCreditsScreen();
        break;
    case "startGame":
        playSound("hit1"), userInput.removeHitArea("startGame"), initGame();
        break;
    case "backFromCredits":
        playSound("hit1"), userInput.removeHitArea("backFromCredits"), initStartScreen();
        break;
    case "moreGames":
    ...
    case "gameTouch":
        if (gameTouchState >= 3)
            return;
        if (b.isBeingDragged && !b.hasLeft)
            2 == gameTouchState && (targAimX = b.x, targAimY = b.y), arrow.alpha = 1;
        else if (b.isDown)
            TweenLite.killTweensOf(oPosData), toggleHudButs(!1), b.x < cueBall.x + 40 && b.x > cueBall.x - 40 && b.y < cueBall.y + 40 && b.y > cueBall.y - 40 && (gameTouchState = 2, aimX = targAimX = b.x, aimY = targAimY = b.y, cueBall.changeState("aiming")), arrow.alpha = b.hasLeft ? .5 : 1;
        else {
            if (toggleHudButs(!0), 2 == gameTouchState && arrow.scaleX > .05)
                return gameTouchState = 3, arrow.takeShot(cueBall), arrow.scaleX < .5 ? playSound("hit1") : arrow.scaleX < .8 ? playSound("hit2") : playSound("hit3"), void 0;
            gameTouchState = 0, "waiting" != cueBall.state && cueBall.changeState("waiting")
        }
        break;
    case "quitFromEndLevel":
        playSound("hit1"), userInput.removeHitArea("quitFromEndLevel"), userInput.removeHitArea("nextGame"), userInput.removeHitArea("moreGames"), initStartScreen();
        break;
    case "nextGame":
        playSound("hit1"), userInput.removeHitArea("quitFromEndLevel"), userInput.removeHitArea("nextGame"), userInput.removeHitArea("moreGames"), initGame();
        break;
    case "mute":
        playSound("hit1"), toggleMute();
        break;
    case "pause":
    case "resumeFromPause":
        playSound("hit1"), toggleManualPause();
        break;
    case "quitFromPause":
        playSound("hit1"), toggleManualPause(), userInput.removeHitArea("pause"), userInput.removeHitArea("gameTouch"), userInput.removeHitArea("quitFromPause"), userInput.removeHitArea("resumeFromPause"), userInput.removeHitArea("moreGamesFromPause"), initStartScreen()
    }
}
...
function initGameEnd() {
    gameState = "gameEnd", 1 == audioType && (musicTween.kill(), musicTween = TweenLite.to(music, 2, {
        volume: .5,
        ease: "Linear.easeNone"
    })), playSound("gameEnd"), userInput.removeHitArea("pause"), userInput.removeHitArea("gameTouch");
    var a = {
            oImgData: assetLib.getData("uiButs"),
            aPos: [660, 320],
            id: "play"
    ...
    userInput.addHitArea("quitFromEndLevel", butEventHandler, null, "image", c), userInput.addHitArea("nextGame", butEventHandler, null, "image", a), userInput.addHitArea("moreGames", butEventHandler, null, "image", b);
    var d = new Array(a, c, b);
    panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("scoreNumbers"), gameState, d, canvas.width, canvas.height), panel.oScoreData = hud.oData, panel.startTweenEndLevel(), previousTime = (new Date).getTime(), updateGameEndEvent()
}
...
function ballCallback(a, b) {
    ...
    case "moveEnded":
        for (var c = !0, d = 0; d < aBalls.length; d++)
            if ("moving" == aBalls[d].state) {
                c = !1;
                break
            }
        if (c) {
            for (var e = !0, d = 0; d < aBalls.length; d++)
                if ("waiting" == aBalls[d].state && "cueBall" != aBalls[d].oData.type || 1 == aBalls.length) {
                    e = !1;
                    break
                }
            if (e)
                return;
            for (var d = 0; d < aBalls.length; d++)
                "waiting" == aBalls[d].state && "cueBall" == aBalls[d].oData.type && aBalls[d].changeState("indicating");
            aimX = targAimX = cueBall.x, aimY = targAimY = cueBall.y, arrow.renderFunc = arrow.renderAim, gameTouchState = 0, playSound("highlight"), oPosData.prevBallX = cueBall.trackX, oPosData.prevBallY = cueBall.trackY, shotsSinceLastPot > 0 && (hud.oData.multiplier = 10, streak = 0), shotsSinceLastPot++
        }
        break;
    case "holeEnded":
        for (var d = 0; d < aBalls.length; d++)
            if (aBalls[d].removeMe) {
                if (!("cueBall" == aBalls[d].oData.type && aBalls.length > 1)) {
                    aBalls.splice(d, 1);
                    break
                }
                for (var f = !1, g = 0, h = aBalls[d].startX, i = aBalls[d].startY; !f;) {
                    for (var j = 0; j < aBalls.length; j++) {
                        if (h > aBalls[j].trackX - aBalls[j].radius && h < aBalls[j].trackX + aBalls[j].radius && i > aBalls[j].trackY - aBalls[j].radius && i < aBalls[j].trackY + aBalls[j].radius) {
                            h = aBalls[d].startX + 160 * Math.random() - 80, i = aBalls[d].startY + 160 * Math.random() - 80, f = !1;
                            break
                        }
                        f = !0
                    }
                ...
                aBalls[d].changeState("reset", {
                    x: h,
                    y: i
                }), aBalls[d].changeState("indicating"), aimX = targAimX = cueBall.x, aimY = targAimY = cueBall.y
            }
...
function updateGameEvent() {
    if (!manualPause && !rotatePause && "game" == gameState) {
        ...
        for (var b = 0; b < aBalls.length; b++) {
            ...
            aBalls[b].update(oPosData.stageX, oPosData.stageY, a), renderSprite(aBalls[b])
        }
        ...
        hud.update(oPosData.stageX, oPosData.stageY, a), hud.render(ctx), renderMuteBut(), requestAnimFrame(updateGameEvent)
    }
}
...
function updateGameEndEvent() {
    if (!rotatePause && "gameEnd" == gameState) {
        var a = getDelta();
        background.updateScroll(a), background.renderScroll(ctx), panel.update(a), panel.render(ctx), renderMuteBut(), requestAnimFrame(updateGameEndEvent)
    }
}
...
function updateTutorialEvent() {
    if (!rotatePause && "tutorial" == gameState) {
        var a = getDelta();
        background.updateScroll(a), background.renderScroll(ctx), panel.update(a), panel.render(ctx), renderMuteBut(), requestAnimFrame(updateTutorialEvent)
    }
}
function getDelta() {
...
function renderSprite(a) {
    ctx.save(), ctx.translate(a.x, a.y), ctx.rotate(a.rotation), ctx.globalAlpha = a.alpha, ctx.scale(a.scaleX, a.scaleY), a.render(ctx), ctx.restore()
}
...
function playSound(a, b) {
    "undefined" == typeof b && (b = 1), 1 == audioType && (sound.volume(b), sound.play(a))
}
function toggleMute() {
...
function toggleManualPause() {
    if (manualPause)
        manualPause = !1, userInput.removeHitArea("quitFromPause"), userInput.removeHitArea("resumeFromPause"), userInput.removeHitArea("moreGamesFromPause"), pauseCoreOff();
    else {
        ...
        var a = {
            ...
            d = new Array(c, a, b);
        userInput.addHitArea("quitFromPause", butEventHandler, null, "image", c), userInput.addHitArea("moreGamesFromPause", butEventHandler, null, "image", b), userInput.addHitArea("resumeFromPause", butEventHandler, null, "image", a), panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("scoreNumbers"), "pause", d, canvas.width, canvas.height), panel.render(ctx), renderMuteBut(), userInput.addHitArea("pause", butEventHandler, null, "rect", {
            aRect: [0, 0, 55, 55]
        }, !0)
    }
}
...
function rotatePauseOff() {
    rotatePause = !1, userInput.removeHitArea("quitFromPause"), userInput.removeHitArea("resumeFromPause"), userInput.removeHitArea("moreGamesFromPause"), pauseCoreOff()
}
...
!function(a) {
    var b = function() {
        function a(a, b, c) {
            this.x = 0, this.y = 0, this.targY = 0, this.incY = 0, this.posY = 0, this.oImgData = a, this.canvasWidth = b, this.canvasHeight = c
        }
        return a.prototype.updateScroll = function(a) {
            this.incY += 5 * a, this.posY -= 8 * this.posY * a
        }, a.prototype.renderScroll = function(a) {
            var b = 40;
            a.drawImage(this.oImgData.img, 0, 0);
            for (var c = 0; b > c; c++)
                a.drawImage(this.oImgData.img, c * (this.canvasWidth / b), 0, this.canvasWidth / b, this.canvasHeight, c * (this.canvasWidth / b), 2 * Math.sin(this.incY + c / 5) - this.posY, this.canvasWidth / b, this.canvasHeight)
        }, a.prototype.render = function(a) {
            a.drawImage(this.oImgData.img, 0, 0)
        }, a
    }();
...
!function(a) {
    var b = function() {
        function a(a, b, c, d, e, f) {
            this.timer = .3, this.endTime = 0, this.posY = 0, this.numberSpace = 28, this.incY = 0, this.oPanelsImgData = a, this.oNumbersImgData = b, this.panelType = c, this.aButs = d, this.canvasWidth = e, this.canvasHeight = f
        }
        return a.prototype.update = function(a) {
            this.incY += 5 * a
        }, a.prototype.startTween1 = function() {
            this.posY = 800, TweenLite.to(this, .8, {
                posY: 0,
                ease: "Back.easeOut"
            })
        }, a.prototype.startTween2 = function() {
            this.posY = 800, TweenLite.to(this, .5, {
                posY: 0,
                ease: "Quad.easeOut"
            })
        }, a.prototype.startTweenEndLevel = function() {
            this.aStarPos = new Array;
            for (var a = 0; a < this.oScoreData.stars; a++)
                this.aStarPos.push({
                    posY: -400,
                    scaleY: 2
            ...
            case "gameEnd":
                var c = 2,
                    ...
                    e = Math.floor(c / (this.oPanelsImgData.img.width / this.oPanelsImgData.oData.spriteWidth)) * this.oPanelsImgData.oData.spriteHeight;
                a.drawImage(this.oPanelsImgData.img, d, e, this.oPanelsImgData.oData.spriteWidth, this.oPanelsImgData.oData.spriteHeight, 0, 0 + this.posY, this.oPanelsImgData.oData.spriteWidth, this.oPanelsImgData.oData.spriteHeight);
                for (var f = this.oScoreData.score, g = 0; g < f.toString().length; g++) {
                ...
                for (var f = this.oScoreData.racks, g = 0; g < f.toString().length; g++) {
                    c = parseFloat(f.toString().charAt(g));
                    var d = c * this.oNumbersImgData.oData.spriteWidth % this.oNumbersImgData.img.width,
                        e = Math.floor(c / (this.oNumbersImgData.img.width / this.oNumbersImgData.oData.spriteWidth)) * this.oNumbersImgData.oData.spriteHeight;
                ...
                for (var f = this.oScoreData.balls, g = 0; g < f.toString().length; g++) {
                    c = parseFloat(f.toString().charAt(g));
                    var d = c * this.oNumbersImgData.oData.spriteWidth % this.oNumbersImgData.img.width,
                        e = Math.floor(c / (this.oNumbersImgData.img.width / this.oNumbersImgData.oData.spriteWidth)) * this.oNumbersImgData.oData.spriteHeight;
                ...
                for (var f = this.oScoreData.streak, g = 0; g < f.toString().length; g++) {
                    c = parseFloat(f.toString().charAt(g));
                    var d = c * this.oNumbersImgData.oData.spriteWidth % this.oNumbersImgData.img.width,
                        e = Math.floor(c / (this.oNumbersImgData.img.width / this.oNumbersImgData.oData.spriteWidth)) * this.oNumbersImgData.oData.spriteHeight;
...
!function(a) {
    var b = function() {
        function a(a, b, c, d) {
            this.x = 0, this.y = 0, this.letterSpace = 13, this.prevSecs = 0, this.oHudImgData = a, this.oTimeNumbersImgData = b, this.oTableNumbersImgData = c, this.oData = d, this.oData.aTime = new Array
        }
        return a.prototype.update = function(a, b) {
            ...
            this.oData.aTime = [b, 10, c, d], 10 > a && d < this.prevSecs && playSound("bell"), this.prevSecs = d
        }, a
    }();
    a.Hud = b
}(Elements || (Elements = {}));
...
!function(a) {
    var b = function(a) {
        function b(b, c, d, e, f, g) {
            a.call(this, b, 24, 26, d.id + "Waiting"), this.radian = Math.PI / 180, this.angle = 0, this.inc = 0, this.ballRadius = 14, this.vx = 0, this.vy = 0, this.m = 1, this.f = 1, this.b = 1, this.oNumbersImgData = c, this.oData = d, this.ballCallback = e, this.trackX = this.startX = this.oData.x, this.trackY = this.startY = this.oData.y, this.p0 = {
                x: this.trackX,
                y: this.trackY
            }, this.p1 = {
                x: this.trackX,
                y: this.trackY
            }, this.canvasWidth = f, this.canvasHeight = g, this.renderFunc = this.renderBall, this.changeState("waiting")
        }
        return __extends(b, a), b.prototype.changeState = function(a, b) {
            switch ("undefined" == typeof b && (b = null), a) {
            case "reset":
                this.fps = 24, this.state = "reset", this.updateFunc = this.updateWaiting, this.renderFunc = this.renderBall, this.removeMe = !1, this.trackX = b.x, this.trackY = b.y, this.x = this.trackX, this.y = this.trackY - 40, this.p0 = {
                    x: this.trackX,
                    y: this.trackY
                }, this.p1 = {
                    x: this.trackX,
                ...
                break;
            case "waiting":
                this.state = "waiting", this.updateFunc = this.updateWaiting;
                break;
            case "indicating":
                this.state = "indicating", this.setAnimType("loop", this.oData.id + "Waiting");
                break;
            case "aiming":
                this.state = "aiming", this.updateFunc = this.updateWaiting, this.setAnimType("loop", this.oData.id + "Moving");
                break;
            case "moving":
                this.state = "moving", this.vx = b.power / 10 * Math.cos(b.angle), this.vy = b.power / 10 * Math.sin(b.angle), this.vz = 1, this.dec = 1, this.setAnimType("loop", this.oData.id + "Moving"), this.p0 = {
                    x: this.trackX,
                    y: this.trackY
                }, this.p1 = {
                    x: this.trackX,
                ...
                break;
            case "rebound":
                this.state = "moving", this.vz = 1, this.dec = 1, this.setAnimType("loop", this.oData.id + "Moving"), this.p0 = {
                    x: this.trackX,
                    y: this.trackY
                }, this.p1 = {
                    x: this.trackX,
                ...
                break;
            case "holed":
                this.state = "holed", this.oData.score = b.score, this.trackX = b.x, this.trackY = b.y, this.scaleX = this.scaleY = 2, this.fps = 18, this.setAnimType("once", "explode"), this.animEndedFunc = function() {
                    this.ballHoled()
                }, this.updateFunc = this.updateWaiting, this.renderFunc = this.renderHoling;
                break;
            case "scoring1":
                this.state = "holed", this.scaleX = this.scaleY = 1, this.scoreX = this.x, this.scoreY = this.y - this.oNumbersImgData.oData.spriteHeight / 2, this.x = this.y = 0, this.scoreScale = .5, this.tween = TweenLite.to(this, 1, {
                    scoreY: this.scoreY - 5,
                    scoreScale: 1,
                    ease: "Back.easeOut",
                    onComplete: this.scoreEnded1,
                    onCompleteParams: [this]
                }), this.updateFunc = this.updateScoring, this.renderFunc = this.renderScoring, playSound("score", .5);
                break;
            case "scoring2":
                this.state = "holed", this.scoreScale = 1, this.tween = TweenLite.to(this, .5, {
            ...
            "undefined" == typeof a && (a = this), "cueBall" != a.oData.type ? a.changeState("scoring1") : a.scoreEnded2(a), a.ballCallback("moveEnded")
        }, b.prototype.scoreEnded1 = function(a) {
            a.changeState("scoring2")
        }, b.prototype.scoreEnded2 = function(a) {
            a.removeMe = !0, a.ballCallback("holeEnded")
        }, b.prototype.update = function(a, b, c) {
            this.updateFunc(a, b, c)
        }, b.prototype.updateMoving = function(b, c, d) {
            a.prototype.updateAnimation.call(this, d), this.vx *= .98, this.vy *= .98, Math.abs(this.vx) < .05 && Math.abs(this.vy) < .05 && this.moveEnded(this), this.x = this.trackX + b, this.y = this.trackY + c
        }, b.prototype.updateScoring = function() {}, b.prototype.updateWaiting = function(b, c, d) {
            ...
            for (var b = this.oData.score, c = 0; c < b.toString().length; c++) {
...
!function(a) {
    var b = function() {
        function a(a, b, c, d) {
            this.x = 0, this.y = 0, this.scaleX = 0, this.scaleY = 1, this.alpha = 1, this.maxLength = 100, this.oArrowImgData = a, this.oCueImgData = b, this.canvasWidth = c, this.canvasHeight = d, this.renderFunc = this.renderAim
        }
        return a.prototype.takeShot = function(a) {
            this.renderFunc = this.renderShot, this.shotHyp = this.hyp, this.shotRot = this.rotation, this.tween = TweenLite.to(this, .15, {
                shotHyp: 0,
                ease: "Quad.easeIn",
            ...
            this.oLineData = e, this.x = a, this.y = b, this.lengthX = this.x - c, this.lengthY = this.y - d, this.hyp = Math.min(e.hyp, this.maxLength), this.scaleX = Math.min(this.hyp / this.maxLength, 1), this.rotation = e.aimRot
        }, a.prototype.render = function(a) {
            this.renderFunc(a)
        }, a.prototype.renderAim = function(a) {
            if (!(this.scaleX < .1)) {
                if (this.oLineData.targBall) {
                    a.save(), a.globalAlpha = this.alpha - (1 - this.scaleX), a.translate(this.oLineData.targBall.x, this.oLineData.targBall.y), a.rotate(this.oLineData.targBallRot);
                    ...
                    a.drawImage(this.oArrowImgData.img, b, c, this.oArrowImgData.oData.spriteWidth, this.oArrowImgData.oData.spriteHeight, -this.oArrowImgData.oData.spriteWidth / 2, -this.oArrowImgData.oData.spriteHeight / 2, this.oArrowImgData.oData.spriteWidth, this.oArrowImgData.oData.spriteHeight), a.restore(), a.save(), a.globalAlpha = this.alpha - (1 - this.scaleX), a.translate(this.oLineData.bounceX, this.oLineData.bounceY), a.rotate(this.oLineData.bounceRot);
                ...
                var d = this.x - this.oLineData.bounceX,
                    ...
                    f = Math.sqrt(d * d + e * e);
                a.save(), a.globalAlpha = this.alpha - (1 - this.scaleX), a.translate(this.x, this.y), a.rotate(this.rotation);
                ...
                a.drawImage(this.oArrowImgData.img, b, c, this.oArrowImgData.oData.spriteWidth, this.oArrowImgData.oData.spriteHeight, 0, -this.oArrowImgData.oData.spriteHeight / 2, Math.max(f - 13, 1), this.oArrowImgData.oData.spriteHeight), a.restore(), a.save(), a.globalAlpha = this.alpha, a.translate(this.x, this.y), a.rotate(this.rotation), a.drawImage(this.oCueImgData.img, 0, 0, this.oCueImgData.oData.spriteWidth, this.oCueImgData.oData.spriteHeight, -this.hyp - this.oCueImgData.oData.spriteWidth - 5, -this.oCueImgData.oData.spriteHeight / 2, this.oCueImgData.oData.spriteWidth, this.oCueImgData.oData.spriteHeight), a.restore()
            }
...
!function(a) {
    var b = function(a) {
        function b(b, c, d, e, f) {
            a.call(this, b, 22, 20, "running"), this.canHit = !0, this.oNumbersImgData = c, this.ballCallback = d, this.canvasWidth = e, this.canvasHeight = f, this.reset(), this.frameInc = Math.ceil(100 * Math.random()), this.animEndedFunc = this.showScore
        }
        return __extends(b, a), b.prototype.reset = function() {
            this.trackX = 550 * Math.random() + 130, this.trackY = 247 * Math.random() + 120, this.scaleX = this.scaleY = 0, this.setAnimType("loop", "running"), TweenLite.to(this, .5, {
                scaleX: 1,
                scaleY: 1,
                ease: "Quad.easeOut"
            }), this.removeMe = !1, this.canHit = !0, this.setPos()
            ...
            a.tween = TweenLite.to(a, 2 * Math.random() + 2, {
                ...
                onComplete: a.setPos,
                onCompleteParams: [a]
            }), a.rotation = Math.atan2(c - a.trackY, b - a.trackX), this.updateFunc = this.updateMoving, this.renderFunc = this.renderMoving
        }, b.prototype.hit = function() {
            this.tween.kill(), this.canHit = !1, this.setAnimType("once", "explode"), this.ballCallback("hitRoach", {
                roach: this
            })
        }, b.prototype.showScore = function() {
            this.scoreScale = .5, this.y -= 75, this.tween = TweenLite.to(this, 1, {
                y: this.y - 5,
                scoreScale: 1,
                ease: "Back.easeOut",
                onComplete: this.scoreEnded1,
                onCompleteParams: [this]
            }), this.rotation = 0, this.updateFunc = this.updateScoring, this.renderFunc = this.renderScoring
        }, b.prototype.scoreEnded1 = function(a) {
            "undefined" == typeof a && (a = this), a.scoreScale = 1, a.tween = TweenLite.to(a, .5, {
                x: a.canvasWidth + 100,
                scoreScale: 2,
                ease: "Back.easeIn",
            ...
            a.prototype.updateAnimation.call(this, d), this.x = this.trackX + b, this.y = this.trackY + c
        }, b.prototype.updateScoring = function() {}, b.prototype.render = function(a) {
            this.renderFunc(a)
        }, b.prototype.renderMoving = function(b) {
            a.prototype.render.call(this, b)
        }, b.prototype.renderScoring = function(a) {
            for (var b = this.roachScore, c = 0; c < b.toString().length; c++) {
                var d = parseFloat(b.toString().charAt(c)),
...
!function(a) {
    var b = function() {
        function a(a, b) {
            this.aLines = new Array, this.aBalls = new Array, this.aLines = a, this.aBalls = b;
        ...
        return a.prototype.drawAll = function(a) {
            ...
            for (b = 0; b < this.aBalls.length; b++) {
                ...
                if ("moving" == c.state) {
                    ...
                    for (var d = 0; d < this.aLines.length; d++) {
                        ...
                        if (e >= 0) {
                            playSound("bumper" + Math.ceil(2 * Math.random())), c.p1.x += this.fi.dx * e, c.p1.y += this.fi.dy * e;
                            var f = {
                                    dx: this.fi.lx,
                                    dy: this.fi.ly,
                                    lx: this.fi.dx,
                    ...
                    for (var h = 0; h < this.aBalls.length; h++)
                        if (b != h && "holed" != this.aBalls[h].state) {
                            var i = this.aBalls[h];
                            this.vc = {}, this.vc.p0 = c.p0, this.vc.p1 = i.p0, this.updateVector(this.vc, a, !0);
                            var j = c.ballRadius + i.ballRadius,
                                k = j - this.vc.len;
                            if (k >= 0) {
                                c.p1.x -= this.vc.dx * k, c.p1.y -= this.vc.dy * k;
                                var l = this.bounceBalls(c, i, this.vc);
                                c.vx = l.vx1, c.vy = l.vy1, i.vx = l.vx2, i.vy = l.vy2, c.changeState("rebound"), i.changeState("rebound"), playSound("ballHit" + Math.ceil(3 * Math.random()))
                            } else if (k >= -50) {
                                this.v3 = new Object, this.v3.p0 = c.p0, this.v3.p1 = {
                                    x: 0,
                                    y: 0
                                }, this.v3.vx = c.vx - i.vx, this.v3.vy = c.vy - i.vy, this.updateVector(this.v3, a);
                                ...
                                this.vn.p0 = n, this.vn.p1 = i.p0, this.updateVector(this.vn, a, !0);
                                var o = j - this.vn.len;
                                if (o > 0) {
                                    ...
                                    if (this.p3 = {
                                        ...
                                        c.p1 = {
                                            ...
                                            p0: c.p1,
                                            p1: i.p1
                                        }, this.updateVector(this.vc, a, !0), l = this.bounceBalls(c, i, this.vc), c.vx = l.vx1, c.vy = l.vy1, i.vx = l.vx2, i.vy = l.vy2, this.makeVector(i), this.makeVector(c), c.changeState("rebound"), i.changeState("rebound"), playSound("ballHit" + Math.ceil(3 * Math.random()))
                                    }
                                }
                            }
                        }
            ...
            "undefined" == typeof c && (c = !1), b = .0167, 1 == c ? (a.vx = a.p1.x - a.p0.x, a.vy = a.p1.y - a.p0.y) : (a.p1.x = a.p0.x + 60 * a.vx * b, a.p1.y = a.p0.y + 60 * a.vy * b), this.makeVector(a)
...
!function(a) {
    var b = function() {
        function a(a, b, c) {
            this.aLines = new Array, this.aBalls = new Array, this.oLineData = {
                targBall: null,
                targBallRot: 0,
                bounceX: 0,
                bounceY: 0,
                ...
                hyp: 0,
                aimRot: 0
            }, this.aLines = a, this.aBalls = b, this.cueBall = c
        }
        return a.prototype.checkLine = function(a, b, c, d, e) {
            var f = this.cueBall.x - c,
                g = this.cueBall.y - d,
                h = Math.abs(f / g);
            this.oLineData.hyp = Math.sqrt(f * f + g * g), this.oLineData.aimRot = Math.atan2(g, f), Math.abs(f) > Math.abs(g) ? (f = f > 0 ? 1 : -1, g = g > 0 ? 1 / h : -1 / h) : (g = g > 0 ? 1 : -1, f = f > 0 ? 1 * h : -1 * h), this.checkPosX = this.cueBall.x, this.checkPosY = this.cueBall.y, this.oLineData.targBall = null;
            for (var i = 0; 700 > i; i++) {
                this.oLineData.targBall = null;
                for (var j = 0; j < this.aBalls.length; j++)
                    if (this.aBalls[j] != this.cueBall) {
                ...
                if (this.checkPosX > 687 || this.checkPosX < 113 || this.checkPosY < 103 + e || this.checkPosY > 377 + e) {
                    this.oLineData.bounceX = this.checkPosX, this.oLineData.bounceY = this.checkPosY;
                    break
                }
                this.checkPosX += f, this.checkPosY += g
            }
...
!function(a) {
    var b = function() {
        ...
        return a.prototype.clearData = function() {
            this.aLevelStore = new Array, this.aLevelStore.push(0), this.aLevelStore.push(0);
            for (var a = 0; a < this.totalLevels - 1; a++)
                0 == (a + 2) % 4 ? this.aLevelStore.push(5) : this.aLevelStore.push(1), this.aLevelStore.push(0)
